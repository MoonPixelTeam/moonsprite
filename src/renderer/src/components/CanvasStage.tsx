...
import { animationMaskAt, cachedLayerContentBounds, createId, getActiveLayer, getLayerIdsInGroup, isLayerEffectivelyLocked, isLayerEffectivelyVisible, layerContentBounds, layerIndexAt, layerMaskDisplayColor, readLayerColor, readLayerColorAt, readLayerMaskDisplayColorAt, readLayerVisibleColorAt, renderLayerMaskRegion, resolveLayerCanvasColor } from '@/core/document-model'
import { compositeRegion, createCompositePointReplacementSampler, createCompositePointSampler, createNormalCompositePointReplacementSampler, createNormalCompositePointSampler, expandLayerStyleInvalidationRect } from '@/core/document-composite'
import { beginPixelEdit, mergePixelEdits, recordPixel, revertPixelEdit } from '@/core/history'
import { beginCanvasToolGesture, clearCanvasToolGestures, endCanvasToolGesture } from '@/core/canvas-tool-gesture-lock'
import { blendOver, packColor, TRANSPARENT } from '@/core/raster'
import { applyGradient, constrainGradientEndpoint, gradientRegionSelection, resolveRadialGradientGeometry, type GradientGeometryOptions } from '@/core/gradient'
...
import { DEFAULT_GRID_SETTINGS, gridCellBoundsAt, snapPointToGrid, snapSelectionBoundsToGrid, snapSelectionTranslationToGrid } from '@/core/grid'
import { alignmentThresholdForZoom, resolveAlignment } from '@/core/alignment'
import { appendPerfectPixelSegment, bezierCurvePixelPoints, filledPolygonPathPixelPoints, filledShapePathPixelPoints, lineShapePixelPoints, paintShape, paintShapePixelPoints, perfectPixelPathPoints } from '@/core/tools-shapes'
import { applySelectionTransform, applySelectionTranslationPreview, captureSelectionTransform, restoreSelectionTranslationPreview, type SelectionTransformLayerState, type SelectionTransformSource } from '@/core/tools-selection-transform'
import { brushStampAnchor, brushStrokeInvalidationRects, inheritBrushPaintBaseline, paintBrush, paintBrushPath, paintLine, solidBrushPreviewRowSpans } from '@/core/tools-brush'
...
import { ISO_VIEW_PREFERENCES_PREVIEW_EVENT, loadEditorPreferences, parseIsoViewPreferences, type BrushPreviewMode, type CheckerboardPreferences, type CursorScale, type EyedropperMagnifierStyle, type GridColorPreferences, type IsoViewPreferences, type OnionSkinPreferences, type RotationIndicatorPosition, type SelectionPreviewColorMode, type SymmetryAxisPreferences, type TabletPreferences, type WheelZoomMode, type ZoomToolDragMode } from '@/core/file-preferences'
import { preserveViewOnViewportChange, type ViewportPlacement, clampCanvasViewPan, displayedCanvasCenter, documentPointFromViewportPoint, documentPointFromViewportPointContinuous, mirrorViewportPoint, rotateViewAroundViewportPoint, rotateViewportPoint, rotationIndicatorFitsCanvas, rotationIndicatorPointBetweenPointerAndCanvasCenter, rotationIndicatorPointLeftOfPointer, snapViewRotation, unrotatedViewportPoint, unrotateViewportPoint, viewCanvasOrigin, viewPanDeltaFromScreen, viewRotationPivot, zoomViewAroundViewportPoint } from '@/core/view-geometry'
import { createCanvasRenderPlan, deviceAlignedCanvasRect, deviceAlignedCoordinate, deviceAlignedDocumentPointAtViewport, deviceAlignedPixelRect } from '@/core/canvas-render-plan'
...
import { advanceIsoAlignedStrokeSegment, isoGridLineSegment, isoLineEndpoint, snapIsoPointToGridVertex, traceIsoGridPointerEdges, updateIsoAlignedStrokePath } from '@/core/isometric'
...
import { CanvasInputState, PointerPressureAdapter, appendCanvasPathStep, beginBrushSpeedTracking, beginTemporaryCenteredMarqueeResize, brushLineConnectionOverridesTemporaryMove, canvasGestureForPreview, centerMarqueeBoundsAtCreationPoint, centeredShapeBounds, clampCanvasZoom as clampZoom, coalescedPointerClientPoints, constrainFreeTransformCornerToAspectRatio, constrainedTranslation, createCanvasPanDrag, createMarqueeResizeStart, deferredSelectionCommitInvalidationRects, deferredSelectionPreviewMaterializationRequired, drawingSizePreviewTargetForDrag, floatingSelectionCopyMode, isCanvasViewNavigationDrag, isCanvasViewNavigationTool, isPendingCanvasPathGesture, isQuickSelectionSecondPress, marqueeSelectionCommit, normalizeCanvasWheelDelta, paletteSamplingShortcutStartsPrimarySample, playbackCanvasNavigationTool, quickSelectCellDragBounds, redoCanvasPathStep, registerPendingCanvasGestureHistory, resizeRotatedMarqueeBounds, resizeSelectionBounds, resizeTransformedSelectionBounds, resolveMarqueeModifierMode, restoreCanvasDragAfterPan, restoreTemporaryCenteredMarqueeResize, revertCancelledCanvasDragPixelChanges, sampledForegroundColorToAdd, selectionGestureMoved, selectionHitStartsContentMove, selectionInteractionHit, selectionMarqueeUsesConstraint, selectionMovePointerDelta, selectionOverlayFrameForDrag, selectionPivotAfterResize, selectionPivotAtDragPoint, selectionPivotHit, selectionResizeHit, selectionRotationAngle, selectionFreeTransformContentHit, selectionFreeTransformHit, selectionTransformedInteractionHit, selectionTransformDeferredPreviewEnabled, selectionTransformGeometrySource, selectionTransformModifiers, selectionTransformPreviewChanged, shapeBounds, shouldClosePolygonLasso, shouldPreserveLineAnchorAfterNoopDrag, shouldRestartFloatingSelectionForCopy, shouldReuseFloatingSelectionSourceForCopy, shouldStartCanvasPan, snapSelectionRotation, steppedCanvasZoom as steppedZoom, temporaryMoveForCanvasInteractionAllowed, temporaryMoveToolAllowed, temporaryTransformOffset, translatedSelectionRect, translatedSelectionTransformPreviewMask, undoActiveCanvasPathGesture, updateBrushSpeedTracking, viewDragClientDelta, wheelCanvasZoom, zoomDragModeForModifiers, zoomDragTarget, type CanvasDragState as DragState, type CanvasPoint as Point, type QuickSelectionPress, type SelectionHandle, type SelectionHit, type SelectionRotationHandle, type SelectionShearHandle } from '@/core/canvas-input'
import { canvasCursors, canvasToolCursor, colorLuminance, directionalResizeCursors, directionalShearCursors, previewCursorTools, resizeCursors, rotationCursors, selectionCornerResizeCursorForPoints, selectionResizeCursorForHandle, selectionRotationCursorForPosition, selectionShearCursorForDirection, shearCursors, selectionCreationCursor, selectionTransformDragCursor, transparencyColorAt } from '@/core/canvas-visuals'
import { MagicWandWorkerClient } from '@/core/magic-wand-worker'
import { defaultSymmetryCenter, hasSymmetry, moveSymmetryCenter, symmetryAxisDragAllowed, symmetryAxisSegment, symmetrySelection, symmetrySelectionDragDelta, transformSymmetrySelection, type SymmetryAxis } from '@/core/symmetry'
...
import { animationFrameIdsForCellKeys, canvasMoveLayerContentPreview, resolveCanvasMoveAnimationCellKeys, resolveCanvasMoveLayerIds, shouldUseFreeTileInstanceMove } from '@/components/canvas-move-selection'
...
import { activeTilemapCelTarget, applyTilemapDocumentEdit, captureTilemapSelectionMove, previewTilemapSelectionMove, writeTilemapCell } from '@/core/tilemap-document'
import { beginTilemapEdit, expandSelectionToTilemapCells, nearestTileRepeatEquivalent, normalizeSelectionForTileRepeatPreview, tilemapCellBounds, tilemapCellIndexAtPoint, tilemapCellLineIndices, tilemapEditableSelectionAtPoint, tilesetHasOnlyTransparentTile, tileRepeatLineSegments, tileRepeatMappedPointForCopies, wrapDocumentPointForTileRepeat, wrapSelectionMaskForTileRepeat } from '@/core/tilemap'
import { freeTileInstanceAtPoint, freeTileInstanceBounds, freeTileSourceEditTargetAtPoint, freeTileSourceForInstance, freeTileSourceStampOrigin, freeTileTileIdForInstance } from '@/core/free-tile'
...
import { createFreeTileSourceEditRaster, freeTileSelectionForInstanceEdit, freeTileSelectionToEditRaster, freeTileSourceSnapshotFromEditRaster, freeTileTransformTargetToEditRaster, selectionCoversRect, type FreeTileSourceEditRaster } from '@/core/free-tile-edit'
...
import { GradientPreviewSurface, BrushPreviewCompositeCache, BrushPreviewStackCache, GradientCompositePreviewCache, GradientPreviewCoverageCache, shareCanvasToolSettings, SymmetryDragState, PolygonPathPreviewRenderCache, MoveLayerContentPreview, MoveLayerClickFlash, FreeTileInstanceFlash, LineAnchorHistory, SELECTION_PIVOT_ICON_SIZE, selectedTextBoxForSession, SELECTION_PIVOT_ICON_OFFSET, nonContentPreviewDragKinds, brushAngleWithDynamics, brushBaseAngle, EYEDROPPER_MAGNIFIER_BASE_SIZE, EYEDROPPER_MAGNIFIER_VIEWPORT_SIZE, symmetryGuideAxisEnabled, timelineSelectionPrecedesCanvasMarquee, selectionBoundsEqual } from './canvas-stage-helpers'
...
export function CanvasStage({ session: storedSession }: { session: DocumentSession }) {
  ...
  useEffect(() => {
    ...
    const unregisterKeyboard = registerCanvasKeyboard({ isActive: () => useWorkspace.getState().activeId === session.document.id, keyUp: releaseModifierSizing })
  ...
  useEffect(() => {
    ...
    return registerCanvasKeyboard({ isActive: () => useWorkspace.getState().activeId === session.document.id, keyDown: prepareEscapeCancellation })
  ...
  useEffect(() => {
    ...
    const unregisterKeyboard = registerCanvasKeyboard({
      isActive: () => useWorkspace.getState().activeId === session.document.id,
      keyDown: (event) => { quickSelectChordKeyDown(event); keyDown(event) },
      keyUp,
      inactiveModifiers: (event) => {
        inputRef.current.ctrlHeld = event.ctrlKey
        inputRef.current.altHeld = event.altKey
        inputRef.current.shiftHeld = event.shiftKey
      }
    })
    window.addEventListener('blur', blur)
    window.addEventListener('focus', focus)
    document.addEventListener('visibilitychange', visibilityChange)
    return () => { unregisterKeyboard(); window.removeEventListener('blur', blur); window.removeEventListener('focus', focus); document.removeEventListener('visibilitychange', visibilityChange) }
  ...
  useEffect(() => {
    ...
    return registerCanvasKeyboard({ isActive: () => useWorkspace.getState().activeId === session.document.id, keyDown })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  touchNavigationPortsRef.current = {
    read: () => ({ preferences: tabletPreferences, view: { ...session.view, ...liveViewRef.current }, documentSize: session.document, viewportSize: stageSize(), rotationIndicatorPosition }),
    beginPan: (point) => {
      const view = liveViewRef.current
      inputRef.current.drag = createCanvasPanDrag({ x: view.panX, y: view.panY }, point)
      beginPanPreview()
    },
    endPan: (commit) => {
      if (inputRef.current.drag?.kind !== 'pan') return
      inputRef.current.finish()
      if (commit) finishPanPreview()
    },
    preview: (geometry) => {
      const view = { ...session.view, ...geometry }
      liveViewRef.current = view
      applyRotationStyle(view)
      scheduleZoomPreview(view)
    },
    finishPinch: (commitRotation) => {
      const rotation = liveViewRef.current.rotation
      finishZoomPreview()
      if (commitRotation) useWorkspace.getState().setViewForDocument(session.document.id, { rotation })
    },
    constrain: (view, size) => constrainCanvasView({ ...session.view, ...view }, size),
    clampZoom,
