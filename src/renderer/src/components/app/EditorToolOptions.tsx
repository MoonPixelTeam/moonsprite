...
export const EditorToolOptions = memo(function EditorToolOptions({ onOpenColorReplacement }: { onOpenColorReplacement: () => void }) {
  ...
  return <PerformanceProfiler id="EditorToolOptions"><div className="tool-options">
    ...
    {isBrushTool && <>
      ...
      <Tooltip content={t('toolOptions.brushDynamicsDescription')}><button className={`pressure-trigger ${session.brushDynamics.effects.size.sensor || session.brushDynamics.effects.strength.sensor || session.brushDynamics.effects.gradient.sensor || session.brushDynamics.effects.angle.sensor ? 'selected' : ''}`} type="button" aria-expanded={pressureFlyoutOpen} onClick={() => setPressureFlyoutOpen((open) => !open)}>{t('toolOptions.brushDynamics')}<ChevronDown size={14} /></button></Tooltip>
    ...
    {session.tool === 'fill' && fillKind === 'gradient' && <>
      ...
      <div className="gradient-freeform-control"><CheckboxField className="tool-checkbox gradient-freeform-toggle" aria-label={t('toolOptions.gradientFreeform')} checked={Boolean(session.gradientFreeform)} disabled={gradientDither !== 'none'} label={t('toolOptions.gradientFreeform')} onChange={(enabled) => { workspace.setGradientFreeform(enabled); if (enabled) setGradientStopsOpen(true) }} />{session.gradientFreeform && gradientDither === 'none' && <button type="button" className="tool-text-button gradient-freeform-open" onClick={() => setGradientStopsOpen(true)}>{t('toolOptions.gradientFreeform')}</button>}</div>
