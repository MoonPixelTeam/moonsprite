...
export const toolOptionsRenderKey = (session: DocumentSession | null): string => {
  ...
  return [
    ...
    session.inkMode,
    session.syncInkAcrossTools ? 1 : 0,
    ...
    session.drawFromCanvasCenter ? 1 : 0,
    session.drawingAnchor?.x ?? 0.5,
    session.drawingAnchor?.y ?? 0.5,
    session.drawingAnchorVisible ? 1 : 0,
    ...
    session.freeTransformActive ? 1 : 0,
