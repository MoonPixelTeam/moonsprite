...
export const ExportDialogHost = forwardRef<ExportDialogHandle, Props>(function ExportDialogHost({defaultFileDirectories, exportScalePresets}, ref) {
  ...
  const chooseExportDirectory = async (): Promise<void> => {
    const result = await chooseExportLocation(window.moonSprite, exportForm.directory || defaultFileDirectories.exportDirectory, exportForm.name, exportForm.format)
    if (result) setExportForm(current => ({ ...current, ...result }))
    setExportPathMenuOpen(false)
  }
  ...
  const stillFrameRange = !projectFormat && exportForm.format !== 'gif' && (exportTarget === 'document' || exportTarget === 'frames') && (session?.document.animation?.frames.length ?? 1) > 1
  const frameRangeValue = stillFrameRange && exportTarget === 'document' ? 'current' : gifFrameRangeValue
  const changeFrameRange = (value: string): void => setExportForm(current => ({
    ...current,
    ...(stillFrameRange ? { target: value === 'current' ? 'document' : 'frames' } : {}),
    gifFrameRange: value.startsWith('loop-section:') ? 'loop-section' : value === 'range' ? 'range' : 'all',
    gifLoopSectionId: value.startsWith('loop-section:') ? value.slice('loop-section:'.length) : undefined
  }))
  ...
  return <>    {exportOpen && <div className="modal-backdrop modal-overlay-backdrop" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget) setExportOpen(false) }}>
      <ModalShell as="form" storageKey="export-layout-v2" fitContentKey={`${exportForm.format}:${exportTarget}:${exportForm.gifFrameRange ?? 'all'}`} defaultWidth={520} defaultHeight={520} minWidth={420} minHeight={360} maxWidth={640} maxHeight={760} resizable={false} className="export-modal" onSubmit={(event) => {
        ...
        <div className="modal-body component-scrollbar export-modal-body">
          ...
          {(exportForm.format === 'gif' || exportTarget === 'frames' || stillFrameRange) && <section className="gif-export-options">
            <FormField label={t('app.export.gifRange')}><ThemedSelect value={frameRangeValue} groups={[{ label: t('app.export.gifRange'), options: [...(stillFrameRange ? [{ value: 'current', label: t('app.export.currentFrame') }] : []), { value: 'all', label: t('app.export.gifAllFrames') }, { value: 'range', label: t('app.export.gifFrameRange') }, ...exportLoopSections.map((section) => ({ value: `loop-section:${section.id}`, label: t('app.export.gifLoopSection', { name: section.name }) }))] }]} label={t('app.export.gifRange')} onChange={changeFrameRange} /></FormField>
