...
export function ExtensionDialogForm({extensionId,windowId,resourceId,onClose}:Props) {
 ...
 const receive=async(message:unknown)=>{const next=message as View;if(!Array.isArray(next.nodes))return;setView(next);if(pending.current&&next.result?.requestId===pending.current){pending.current='';setValues({});try{if(next.result.file){const saved=await saveExtensionFile(next.result.file);setView(current=>({...current,status:saved?t('extension.fileExported'):t('extension.exportCancelled')}))}}catch(error){setView(current=>({...current,status:String(error)}))}finally{setBusy(false)}}}
