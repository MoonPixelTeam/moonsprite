...
export function ExtensionDialogForm({extensionId,windowId,resourceId,onClose}:Props) {
 ...
 const render=(node:Node,depth=0):ReactNode=>{
  ...
  switch(node.type){
   ...
   case 'slot':return <section className="extension-form-slot" aria-label={node.label} key={node.id}><div><h3><Tooltip content={typeof node.tooltip==='string'?node.tooltip:undefined}><span>{node.label}</span></Tooltip></h3><p>{node.description}</p></div>{node.children?.slice(0,16).map(child=>render(child,depth+1))}</section>
