...
export function ExtensionDialogForm({extensionId,windowId,resourceId,onClose}:Props) {
 ...
 const render=(node:Node,depth=0):ReactNode=>{
  ...
  switch(node.type){
   case 'dialog':return createPortal(<div className="modal-backdrop" data-extension-prompt={node.id} onPointerDown={event=>event.stopPropagation()} onFocusCapture={event=>event.stopPropagation()}><ModalShell storageKey={'extension-prompt:'+extensionId+':'+node.id+(node.compact?':compact':'')} className={node.compact?'extension-compact-prompt':''} fitContent fitContentKey={node.compact?JSON.stringify(node.children):undefined} defaultWidth={440} defaultHeight={node.compact?180:440} minWidth={280} minHeight={node.compact?120:240} role="dialog" aria-modal="true" aria-label={node.label}><DialogHeader title={node.label||''} closeLabel={t('common.cancel')} onClose={()=>send(node.action||{})}/><div className="extension-dialog-form" style={{flex:1,minHeight:0}}><div className="modal-body">{node.children?.slice(0,100).map(child=>render(child,depth+1))}</div></div></ModalShell></div>,document.body,node.id)
   case 'message':return <p key={node.id} className="extension-form-message">{node.label}</p>
   ...
   case 'slot':return <section className="extension-form-slot" onContextMenu={event=>{if(node.contextAction&&!disabled){event.preventDefault();event.stopPropagation();setValues({});send(node.contextAction)}}} aria-label={node.label} key={node.id}><div className="extension-form-slot-heading"><h3><FormCopy text={node.label} description={node.tooltip}/></h3><p><FormCopy text={node.description}/></p></div><div className="extension-form-slot-actions">{node.children?.slice(0,16).map(child=><div className="extension-form-slot-action" key={child.id}>{render(child,depth+1)}</div>)}</div></section>
   ...
   case 'button':return <Button key={node.id} title={labelHint(node)} aria-label={node.label} aria-pressed={node.selected} variant={node.primary?'primary':'quiet'} disabled={disabled} onClick={()=>send(node.action||{})}><FormCopy text={node.label} description={node.tooltip}/></Button>
