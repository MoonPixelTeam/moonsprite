  ...
  it.each([4.125, 16, 64.013])('keeps interior pixel edges independent of the visible crop at zoom %s', (zoom) => {
    ...
    for (const dpr of [1, 1.25, 1.501]) for (const pan of [-120.37, 0.2, 12.9]) {
      ...
      for (const [start, count] of [[0, 192], [3, 35], [11, 17]]) {
        ...
        const [, sx, , sw, , dx, , dw] = drawImage.mock.lastCall! as unknown as [unknown, number, number, number, number, number, number, number, number]
        ...
        for (let pixel = start + 1; pixel < start + count; pixel++) {
