...
it('refreshes the preference snapshot together without persisting an iso preview', () => {
  ...
  act(() => {saveEditorPreferences({...initial, brushSizeWheelReversed: true, wheelZoomEnabled: !initial.wheelZoomEnabled, drawingBrushPreviewEnabled: !initial.drawingBrushPreviewEnabled}); window.dispatchEvent(new Event('moonsprite:preferences-changed'))})
  expect(result.current.brushSizeWheelReversed).toBe(true)
  expect(loadEditorPreferences().brushSizeWheelReversed).toBe(true)
