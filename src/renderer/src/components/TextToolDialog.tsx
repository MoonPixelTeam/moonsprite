    ...
    <ModalShell as="form" storageKey="text-tool" defaultWidth={780} defaultHeight={520} minWidth={680} minHeight={450} maxWidth={980} maxHeight={760} className="text-tool-modal" onSubmit={(event) => {
      ...
      <div className="text-tool-body">
        <aside className="text-tool-font-panel" aria-label={t('textTool.font')}>
          ...
          {fontMessage && <p className="text-tool-font-message" role="status" title={fontMessage}>{fontMessage}</p>}
          <div className="text-tool-font-actions">
            {systemFonts.length > 0 ? <ThemedSelect value={systemFonts[0].family} groups={systemFontGroups} label={t('textTool.importFont')} disabled={fontBusy} onChange={useSystemFont} popoverWidth={420} popoverClassName="text-tool-system-font-popover" showCheck={false} showOptionTooltips={false} searchable searchPlaceholder={t('textTool.searchFonts')} renderSelected={() => <span className="text-tool-font-action-copy">{t('textTool.importFont')}</span>} renderOption={(option) => <strong style={{ fontFamily: `"${option.value.replaceAll('"', '')}"` }}>{option.label}</strong>} /> : <button type="button" className="quiet-button" disabled>{t('textTool.importFont')}</button>}
            ...
            <DeleteIconButton className="text-tool-delete-font-button" disabled={fontBusy || selectedFont?.source !== 'imported'} title={t('textTool.deleteFont', { name: selectedFont?.family ?? value.fontFamily })} aria-label={t('textTool.deleteFont', { name: selectedFont?.family ?? value.fontFamily })} onClick={() => void removeSelectedFont()} />
          </div>
        </aside>
