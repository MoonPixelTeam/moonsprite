...
export const activeCelMasksByLayer = (document: SpriteDocument): Map<string, LayerMask> => {
...
export const activeGroupMasksByGroup = (document: SpriteDocument): Map<string, LayerMask> => {
  const timeline = document.animation
  if (!timeline) return new Map()
