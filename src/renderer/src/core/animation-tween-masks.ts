...
export function betweenTweenMask(start: LayerMask | undefined, end: LayerMask | undefined, from: SelectionRect, to: SelectionRect, t: number, maxPixels: number, reserve?: (bytes: number) => void): Pick<LayerMask, 'width' | 'height' | 'offsetX' | 'offsetY' | 'pixels' | 'visible' | 'opacity'> {
