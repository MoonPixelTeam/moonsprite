...
- `pets[]` 是由宿主独占渲染的陪伴宠物贡献。每项声明透明 PNG 图集、帧宽高和具名动画；状态只能是 `show`、`idle`、`drag`、`inspect`、`save`、`export-complete`、`unsaved-reminder`、`break-reminder` 或 `sleep`。安装时平台层验证 PNG、帧尺寸、帧索引与包内路径，Renderer 只能以扩展和宠物 ID 请求图集，不能获得扩展目录。宠物窗口是主窗口拥有的透明窗口，不使用全局置顶；位置、显示、缩放和提醒偏好只保存在本机，不进入工程或撤销历史。
- 单个扩展最多声明 64 个命令、16 个栏目、32 个现有菜单贡献、16 个新增顶层菜单、16 个工具贡献和 8 个宠物贡献；每个栏目或菜单贡献最多引用 32 个命令。各贡献 ID 在自己的命名空间内不允许仅靠大小写区分；命令入口必须存在于包内，所有贡献只能引用同一清单中已声明的命令。
