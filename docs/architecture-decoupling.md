...
| 组件 | 当次拆分前 → 拆分后行数 | 当次最大函数行数 | 当次直接 useRef / useState / useEffect |
| --- | --- | --- | --- |
...
当前源码行数快照（2026-09-19）：CanvasStage 1530 行，LayersPanel 1155 行，App 926 行。上表保留当次重构前后的测量，后续功能增加不倒改历史数据；行数本身不证明性能或解耦质量。
...
以下测试数量和日志属于对应重构轮次的历史验证，不是 2026-09-19 文档审校重新执行的结果。当前功能变更仍按 AGENTS.md 进行定向验证。
