  ...
  return <>
    <div className="competition-gallery">
      {works.map((work) => <figure className="competition-work" key={work.id}>
        <button type="button" className="competition-work-image" aria-label={`${language === 'zh' ? '放大作品：' : 'Enlarge artwork: '}${work.title[language]}`} onClick={() => setPreview({ src: work.image, title: work.title[language], description: awardLabels[work.award][language] })}>
