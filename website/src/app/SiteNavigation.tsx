      ...
      <div className="menubar">
        ...
        <div className="menubar-utils">
          <span className="cart-slot">
            <button type="button" className="icon-button" aria-label={language === 'zh' ? '购物车' : 'Cart'} title={language === 'zh' ? '购物车' : 'Cart'} onClick={openCartFromHeader}><PixelCart /></button>
            {cart.count > 0 && <span className="cart-badge">{cart.count}</span>}
          </span>
          <a
            className="icon-button account-button"
            title={account ? `${t.accountPage.signedInAs}: ${account.name}` : t.accountPage.title}
            ...
            {account ? <PixelUserSignedIn /> : <PixelUserSignedOut />}
          </a>
          <button type="button" className="icon-button" title={theme === 'dark' ? t.common.themeToLight : t.common.themeToDark} aria-label={theme === 'dark' ? t.common.themeToLight : t.common.themeToDark} onClick={() => setTheme((value) => value === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? <PixelSun /> : <PixelMoon />}</button>
          <div className="lang-menu" ref={langMenuRef}>
            <button className="language-button icon-button" aria-label={language === 'zh' ? '切换语言' : 'Change language'} title={language === 'zh' ? '切换语言' : 'Change language'} type="button" onClick={() => setLangOpen((value) => !value)} aria-haspopup="listbox" aria-expanded={langOpen}><PixelLanguage /></button>
          ...
          <button className="menu-button" type="button" onClick={() => setMenuOpen((value) => !value)} aria-expanded={menuOpen} aria-controls="site-navigation" aria-label={menuOpen ? t.nav.close : t.nav.menu}>{menuOpen ? <X /> : <Menu />}</button>
        </div>
      </div>
    </header>
  )
